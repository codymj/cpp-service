...
void Settings::load(const std::string& config_file) {
	...
	try {
		...
		// server.port
		if (int64_t(doc["server"]["port"])<0 || int64_t(doc["server"]["port"])>65535) {
			throw std::invalid_argument("server.port must be between 0 and 65535");
		}
		properties["server_port"] = (int)int64_t(doc["server"]["port"]);

		// server.read_timeout
		if (int64_t(doc["server"]["read_timeout"])<0) {
			throw std::invalid_argument("server.read_timeout must be 0 seconds or more");
		}
		properties["server_read_timeout"] = (int)int64_t(doc["server"]["read_timeout"]);

		// server.write_timeout
		if (int64_t(doc["server"]["write_timeout"])<0) {
			throw std::invalid_argument(
				"server.write_timeout must be 0 seconds or more");
		}
		properties["server_write_timeout"] = (int)int64_t(doc["server"]["write_timeout"]);

		// server.idle_timeout
		if (int64_t(doc["server"]["idle_timeout"])<0) {
			throw std::invalid_argument(
				"server.idle_timeout must be 0 seconds or more");
		}
		properties["server_idle_timeout"] = (int)int64_t(doc["server"]["idle_timeout"]);
		...
		// database.port
		if (int64_t(doc["database"]["port"])<0 || int64_t(doc["database"]["port"])>65535) {
			throw std::invalid_argument("database.port must be between 0 and 65535");
		}
		properties["database_port"] = (int)int64_t(doc["database"]["port"]);
		...
		// database.password
		std::string_view password_env{std::string_view(doc["database"]["password"])};
		if (std::string_view(doc["database"]["password"]).empty()) {
			throw std::invalid_argument("database.password cannot be empty");
		}
		std::string tmp{password_env.begin(), password_env.end()};
		...
		if (int64_t(doc["database"]["connection_max_lifetime"])<0) {
			throw std::invalid_argument("database.connection_max_lifetime must be 0 or greater");
		}
		properties["database_connection_max_lifetime"] = (int)int64_t(doc["database"]["connection_max_lifetime"]);

		// database.max_open_connections
		if (int64_t(doc["database"]["max_open_connections"])<0) {
			throw std::invalid_argument("database.max_open_connections must be 0 or greater");
		}
		properties["database_max_open_connections"] = (int)int64_t(doc["database"]["max_open_connections"]);

		// database.max_idle_connections
		if (int64_t(doc["database"]["max_idle_connections"])<0) {
			throw std::invalid_argument("database.max_idle_connections must be 0 or greater");
		}
		properties["database_max_idle_connections"] = (int)int64_t(doc["database"]["max_idle_connections"]);
